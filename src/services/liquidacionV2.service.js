...
async function polizasDisponibles() {
  return query(
    `SELECT p.codigo, p.nombre_poliza, p.fecha, p.estado,
       ...
       FROM man_poliza p
      WHERE UPPER(p.estado) IN ('CERRADA', 'CERRADA SIN LIQUIDAR')
        AND NOT EXISTS (
          SELECT 1
...
async function validarPolizaParaGenerar(idPoliza) {
  ...
  if (!['CERRADA', 'CERRADA SIN LIQUIDAR'].includes(String(poliza.estado || '').toUpperCase())) {
    throw errorNegocio(`La póliza debe estar CERRADA para liquidarse (estado actual: ${poliza.estado}).`);
  }
...
async function vistaPrevia(idPoliza) {
  ...
  const [viajes, anticipos, vales, sobregiros, parametro] = await Promise.all([
    query(
  ...
  const resultado = ids.map((idTransportista) => {
    ...
    const totalFacturar = money(baseGravable - valorImpuesto);
...
async function detalleLiquidacion(idLiquidacion) {
  ...
  const [detalles, vales] = await Promise.all([
    query(
      `SELECT d.*, t.nit, t.nombre_comercial
    ...
    query(
      `SELECT d.correlativo, d.num_vale, d.id_transportista, d.fecha,
...
async function generar(idPoliza, idLiqOrigen = null) {
  const id = idValido(idPoliza, 'id_poliza');
...
async function historial(filtros = {}) {
  ...
  return query(
    `SELECT l.correlativo AS id_liquidacion, l.num_liquidacion, l.id_poliza,
            ...
            d.cantidad_viajes, d.valor_viajes, d.valor_diesel, d.valor_anticipos,
            d.base_gravable, d.porcentaje_impuesto, d.valor_impuesto,
  ...
  return query(
    `SELECT l.correlativo AS id_liquidacion, l.num_liquidacion, l.id_poliza,
      ...
      WHERE l.id_transportista IS NULL AND COALESCE(l.revertida, 0) = 0
        AND UPPER(l.estado) = 'LIQUIDADO' ${filtro}
...
async function revertir(idLiquidacion, usuario, motivo) {
  ...
  await query('CALL sp_revertir_liquidacion(?, ?, ?)', [id, usuario || 'sistema', razon]);
...
async function registrarAbono(data, usuario) {
  ...
  const saldo = await queryOne(
    `SELECT COALESCE(SUM(valor_sobregiro - valor_abonado), 0) AS saldo
       FROM pro_sobregiro_transportista
